# OKB 構成案（設計方針）

前作 `tsumugilabs/elevator-action` の `docs/HANDOFF.md` とエンジンを土台に、
**狙撃を主役にした護衛シューティング**として OKB を立ち上げる設計メモ。

## 1. コンセプト確定

| 項目 | 内容 |
| --- | --- |
| 仮タイトル | **OKB** |
| ジャンル | ストーリー型・護衛スナイパー |
| プレイヤー | 屋上の**スナイパー**（地上キャラは操作しない） |
| 護衛対象 | **前作の主人公**（潜入エージェント）。**自律的に**出口へ歩く |
| 主役の動詞 | 妨害に来る**敵をタップ狙撃**して護衛を守る |
| 演出方針 | 狙撃演出は残す／**GUILTY 劇画カットは基本省略**、**ボス限定**で発動 |
| 物語提示 | ステージ前後に**忍者龍剣伝ライク**の劇中劇（グラフィック＋テキスト） |
| 技術 | 素の JS + HTML5 Canvas、ビルド不要、Web Audio 合成音、モバイル対応 |

**転換点**：前々案（現れた標的を撃つ判断ゲーム）から、ユーザー指定により
**「護衛対象を狙撃で守る」**へ中心ループを作り替えた。前作主人公を護衛対象に
据えることで、two-game のナラティブが繋がる。

## 2. 中心ループ

全4章。第1〜3章＝**護衛（横スクロール）**、終章＝**裏切りの狩り（hunt）**。

```
CUTSCENE(章導入) → PLAY → CUTSCENE(章締め) → 次章 … → ENDING

escort（第1〜3章）: 護衛対象が自律歩行で EXIT へ（カメラ追従で1画面超のルート）
    敵が妨害に出現 → タップ狙撃 → 被弾/接触で護衛HP減 → EXIT到達で成功／HP0で失敗
hunt（終章）: 元・護衛対象が寝返り、物陰を渡って ESCAPE へ逃走
    「物陰→覗き見→ダッシュ」を反復。露出した隙をタップ狙撃（決着でGUILTY）
    逃がす（ESCAPE到達）と失敗
```

### スコープ狙撃（視点システム）
「ただタップして撃つ」をやめ、**スナイパーらしい二段階操作**にした：
- **素の視界（hip）**：敵・護衛は `HIP_ACTOR≈0.62` で小さく描画＝**遠く**に見える。
  この状態では発砲不可（撃つと「スコープを覗け」の促し）。
- **スコープ（scoped）**：ピンチイン／ダブルクリックで ON。照準点を中心に
  `SCOPE_Z≈3` 拡大した**円形レンズ**を重ね（外側は暗転＝トンネルビジョン）、
  **カメラを固定**。レンズ内で狙って**クリック／タップ発砲**。ピンチアウト／
  再ダブルクリックで OFF。
- 実装：`drawWorldContent(actorScale)` を hip は縮小・scope はクリップ＋
  ズーム変換で 1.0 再描画。当たり判定はワールド原寸なので拡大表示と一致。
  入力は `input.js` が pinch（2ポインタ距離）と double-click（タップ間隔）を
  ジェスチャ化。

### トリガー分離と手動リロード
「照準タップ＝発砲」だと発砲の瞬間に狙点がズレる問題があったため、**トリガーを
照準から分離**した：
- 画面キャンバスは**照準とスコープ切替専用**（タップでは発砲しない）。
- **SHOT ボタン（Space / F）**で現在の照準に発砲。**RELOAD ボタン（R）**で再装填。
- **手動リロード**：1発撃つと薬室は空（`game.loaded=false`）。RELOAD は押すと
  **強制的に素の視界へ**戻し（`scopeOut`）、`RELOAD_TIME`(~0.5s) 後に装填完了。
  → 「構える→撃つ→下ろして込める」の間合い。
- 入力は取りこぼし防止のため**ラッチ式**（`Input.consume(action)`）：キー/ボタンの
  押下エッジをキューに積み、1フレーム未満の速い操作も確実に発火。

### 横スクロールカメラ
全座標はワールド座標。カメラ `camX` が対象を追い、`[0, worldW-画面幅]` にクランプ。
描画は `translate(-camX)`、タップは `world = screen + camX` で変換。ガンナーは
**護衛対象が起動距離に入ると**アクティブ化（画面外からの理不尽な発砲を防ぐ）。

### 誤射ペナルティと持ち越し（リスク＆リワード）
護衛章で**護衛対象を撃つ＝誤射**（`game.misfires`、走行中は累積）。効果：
- 1回ごとに護衛対象の速度 ×0.8（恒久・段階的に低下）＋ ラッシャーを追加投入。
- **3回で即失敗**（`missionFail`）。
- 終章の裏切り者は護衛対象と同一人物なので、**誤射数（最大2）ぶん HP と dash 速度を減算**
  （HP `3-wounds`、dash `4.6-1.0*wounds`、覗き見時間 `30+15*wounds`）。
  → 「今つらいが後で効く」逆説。3回=失敗ゆえ持ち越し上限は2＝終章HP最小1。

### 狩り（hunt）の“隙”設計
一定速度移動は単調 ―― という指摘を受け、標的を**状態機械**にした：
`hide`(物陰・無敵) → `peek`(覗き見・静止して露出＝主な狙い目) → `dash`(次の物陰へ・
露出だが高速)。露出中だけ被弾判定があり、当てれば**GUILTY 決着**。物陰は描画順で
オクルージョン（隠れ中は物陰の後ろに、露出中は手前に描く）。

### 緊張の設計（決め打ち・調整容易）
- **ボルトアクションのリロード**：1発ごとにクールダウン（既定 42f≒0.7s）。
  連打で全処理させず、**脅威の優先順位付け**を迫る。空撃ちも1発分消費。
- **通常キルは時間を止めない**：護衛ゲームなので他の脅威が動き続ける。
  タップ即キル＋ヒット演出のみ。**ボスだけ**世界を止めてギルティ演出。
- **敵2種**：`gunner`（予兆「!」→発砲＝護衛HP-1、以後再照準を反復）／
  `rusher`（護衛へ突進、接触で護衛HP-1）。

## 3. 状態機械（前作の `tick()` を踏襲）

```
STATE = { MENU, CUTSCENE, PLAY, RESULT, OVER, ENDING }
game.finisher   // ボスのGUILTY中だけ PLAY を凍結（前作 OKB 13 と同じ作法）
```

## 4. 再利用 / 改修 / 置換

| 前作モジュール | OKB | 区分 |
| --- | --- | --- |
| `game.js` 状態機械・`tick` | `game.js` | 再利用（着せ替え） |
| `game.js` OKB13 演出・`drawGuiltyCut` 一式 | `game.js` | **移植（ボス限定で温存）** |
| `audio.js` `Sound` | `audio.js` | 再利用＋SFX追加・再スコア |
| `input.js` `Input` | `input.js` | 改修（ポインタ照準＋タップ） |
| `entities.js` `overlaps`/AABB | `entities.js` | 再利用（敵タップ判定） |
| `entities.js` `Player/Enemy/moveAndCollide` | `entities.js` | 置換（`Escort`＋`Enemy`、重力なし） |
| `level.js` ビル・エレベーター | `scene.js` | **置換**（護衛ルート＋スポーン表） |
| ― | `cutscene.js` | 新規（劇中劇） |
| ― | `story.js` | 新規（台本データ） |

## 5. データ駆動の拡張点

- **シーン**（`scene.js`）：`{ name, ground, startX, exitX, exit, escortSpeed,
  escortHp, drawBg, spawns[] }`。`spawns` は
  `{ t, x|side, type("gunner"|"rusher"), dir, speed?, boss? }`。
- **劇中劇**（`cutscene.js` + `story.js`）：パネル配列
  `{ bg, figure?, figureX?, speaker?, text }`。背景・立ち絵はベクター描画キー。
- **章**（`story.js`）：`prologue / chapters[{intro,scene,outro}] / ending`。
  → シーン・章・台本は**データ追加だけで拡張**できる。

## 6. 伸びしろ（今後）

- [x] **横スクロール化**（護衛ルートをカメラ追従で長く）
- [x] **裏切りの狩りステージ**（物陰移動＋露出狙撃、GUILTY 決着）
- **敵バリエーション**：人質を取る敵・盾持ち・護衛対象への偽装（誤射誘発）。
- **狩りの強化**：フェイント（覗いてすぐ引っ込む）、複数の逃走ルート、囮。
- **難易度**：`escortSpeed / escortHp / リロード長 / spawn密度 / gunner range`を
  パラメータ化し、前作の「周・面で難易度を返す純関数」思想を踏襲。
- **演出**：命中率ランク、章末リザルト、被弾時の画面効果。
- **縦移動ステージ**：前作エレベーターアクション式の縦スクロール護衛も候補。

## 7. デプロイ運用（前作 HANDOFF を踏襲）

1. `node --check js/*.js` で全 JS 構文チェック。
2. Playwright（headless Chromium）＋ `python3 -m http.server` で表示・通し確認。
3. 単一HTML（Artifact）化：CSS/JS を読み込み順にインライン（CSP対策・画像は data URI）。
   演出は**画像不使用の純ベクター**なので埋め込み不要。
4. Git：開発ブランチ → PR。GitHub Pages（`.nojekyll` あり）。

## 8. 作法（IP/著作権）

- 出自不明の既製画像は公開物へ埋め込まない。キャラ・背景・演出は**自前ベクター**で用意。
- 物語・固有名詞（エージェント／〈カゲロウ〉／〈鴉〉等）は本作のオリジナル。
  「忍者龍剣伝ライク」は**演出形式（劇中劇の作り）**の参照であり、素材の流用ではない。
